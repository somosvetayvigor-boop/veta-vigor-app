...
async function onRequest2(context) {
  ...
  const logs = [];
  const addLog = /* @__PURE__ */ __name((msg) => logs.push(msg), "addLog");
  ...
  const currentDate = mexicoTime.getDate();
  ...
  const { data: inscritos, error: insError } = await supabase.from("perfiles").select("id").not("reto_activo_id", "is", null);
  if (insError) {
  ...
  const sendPush = /* @__PURE__ */ __name(async (segmentIds, titulo, mensaje, link, tipoMensaje) => {
    if (segmentIds.length === 0) return 0;
    addLog(`Intentando enviar [${tipoMensaje}] a ${segmentIds.length} usuarios.`);
    const { data: yaEnviados } = await supabase.from("push_logs").select("user_id").eq("tipo_mensaje", tipoMensaje).in("user_id", segmentIds);
    ...
    if (!TEST_MODE && env.ONESIGNAL_APP_ID && env.ONESIGNAL_REST_API_KEY) {
      try {
        const payload = {
          ...
          url: link
          // Deep link (App/PWA lo maneja si se envía bien)
        };
      ...
      addLog(`[TEST MODE o Faltan APIs] Simulaci\xF3n exitosa a ${aEnviar.length} usuarios.`);
      successCount = aEnviar.length;
    }
    if (successCount > 0) {
  ...
  await sendPush(
    userIds,
    "Tu lugar est\xE1 confirmado",
    "Entra a La Forja y prep\xE1rate para comenzar el 10 de agosto.",
  ...
  if (currentMonth === 8 && currentDate === 9 && currentHour >= 20) {
    await sendPush(
      userIds,
      "Ma\xF1ana comenzamos",
      "Descansa. Tu primera Misi\xF3n estar\xE1 lista al iniciar el d\xEDa.",
      "https://app.vetayvigor.com/reto-21-dias",
      "manana_empezamos"
    );
  }
  if (currentMonth === 8 && currentDate === 10) {
    await sendPush(
      userIds,
      "EMPEZAMOS",
      "Tu primera Misi\xF3n del Reto Vigor 21 ya est\xE1 lista.",
      "https://app.vetayvigor.com/reto-21-dias",
      "empezamos"
    );
  }
  if (currentMonth === 8 && currentDate >= 10 && currentDate <= 30 && currentHour >= 19) {
    const { data: checkinsHoy } = await supabase.from("checkins").select("user_id").eq("fecha", todayStr).in("user_id", userIds);
    const usersConCheckin = (checkinsHoy || []).map((c) => c.user_id);
    const usersSinCheckin = userIds.filter((id) => !usersConCheckin.includes(id));
    await sendPush(
      usersSinCheckin,
      "Tu Misi\xF3n sigue disponible",
      "Todav\xEDa puedes presentarte y completar tu registro.",
      "https://app.vetayvigor.com/",
      `mision_pendiente_${todayStr}`
    );
  }
  addLog("Evaluaci\xF3n terminada.");
...
// ../.wrangler/tmp/pages-xm78be/functionsRoutes-0.16525481982695833.mjs
var routes = [
  {
    routePath: "/api/chat",
    mountPath: "/api",
