...
async function importRoble() {
    console.log("🚀 Iniciando importación de Roble...");
    ...
    sistemaMap['Vigor Corporal'] = sistemaMap['Vigor Corporal (Calistenia)'];

    const data = await parseCSV('roble_rutinas.csv');
    let rutinasCount = 0;
    let puenteCount = 0;
    let ejCount = 0;
    ...
    for (const row of data) {
        ...
        for (let i = 1; i <= 6; i++) {
            const ejName = row[`Ejercicio ${i}`]?.trim() || row[`Ejercico ${i}`]?.trim();
            if (!ejName) continue;
