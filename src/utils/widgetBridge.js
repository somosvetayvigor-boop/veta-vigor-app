...
/**
 * Escribe la racha, la rutina de hoy, el nivel (madera), el último
 * entrenamiento, el progreso semanal y la frase del día a un almacén
 * nativo (SharedPreferences) para que el widget de pantalla de inicio los
 * lea sin pasar por el WebView. No lee el SQLite directo desde Kotlin a
 * propósito -- evita duplicar la lógica de "qué toca hoy" en dos
 * lenguajes. El widget se actualiza solo cada ~30 min (mínimo real de
 * Android), así que esto no necesita ser instantáneo.
 */
export async function actualizarWidget({ racha, rutinaHoy, nivel, ultimoEntrenamiento, progresoSemanal, fraseDelDia }) {
  ...
  try {
    ...
    await Preferences.set({ key: 'fraseDelDia', value: fraseDelDia || '' });
  } catch (e) {
