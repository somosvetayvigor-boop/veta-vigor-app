...
import { MessageCircle, Send, ShieldAlert, Lock, ArrowRight, User, X, ImagePlus, Heart, Award } from 'lucide-react';
...
export default function Comunidad({ session }) {
  ...
  useEffect(() => {
    const verificarNivel = async () => {
      const { data } = await supabase
        .from('perfiles')
        .select('plan_membresia, chat_bloqueado')
      ...
      const tieneAcceso = planActual === 'Socio Fundador Vitalicio' || planActual === 'Plan Platinum' || planActual.toLowerCase().includes('administrador');
  ...
  useEffect(() => {
    ...
    const fetchMensajes = async () => {
      ...
      if (muroData) setMuroFama(muroData);
  ...
  const checkDailyImageLimit = async () => {
    ...
    if (!error && count >= 4) {
      alert("Haz alcanzado el límite diario de 4 fotos por día en la comunidad. ¡Intenta de nuevo mañana!");
      return false;
    }
    return true;
  };
  ...
  const handleReport = async () => {
    ...
    alert('Mensaje reportado. La administración lo revisará pronto.');
    setSelectedMsg(null);
  };
  ...
  if (isBanned) {
    return (
      <div className="container" style={{ padding: '20px', paddingBottom: '100px', display: 'flex', alignItems: 'center', justifyContent: 'center', height: 'calc(100vh - 120px)' }}>
        <div className="card" style={{ textAlign: 'center', padding: '40px 20px', background: 'linear-gradient(145deg, #1a0505 0%, #2a0808 100%)', border: '1px solid rgba(229, 80, 57, 0.3)' }}>
        ...
        <div className="card" style={{ textAlign: 'center', padding: '40px 20px', background: 'linear-gradient(145deg, #15151a 0%, #1a1a24 100%)' }}>
          ...
          <div style={{ borderTop: '1px solid rgba(255,255,255,0.05)', paddingTop: '30px' }}>
            ...
            <p style={{ color: 'var(--text-muted)', fontSize: '0.9rem', lineHeight: '1.5', maxWidth: '300px', margin: '0 auto' }}>
              El Chat está reservado exclusivamente para el <strong>Plan Platinum</strong> y <strong>Socios Vitalicios</strong>.
            </p>
          </div>
        </div>
      </div>
  ...
  return (
    <div style={{ 
      ...
      <div style={{ padding: '15px 20px', backgroundColor: 'rgba(212, 175, 55, 0.05)', borderBottom: '1px solid rgba(212, 175, 55, 0.2)', display: 'flex', alignItems: 'center', gap: '10px' }}>
        ...
        <div>
          <h2 className="gold-gradient-text" style={{ fontSize: '1.8rem', margin: 0 }}>Veta VIP</h2>
          <p style={{ margin: '5px 0 0 0', color: '#888', fontSize: '0.85rem' }}>La élite que no se rinde. (+18)</p>
        </div>
      </div>

      {/* MURO DE LA FAMA BANNER */}
      {muroFama && (
        <div style={{ 
          ...
          <div>
            <h4 style={{ margin: '0 0 3px 0', color: 'var(--accent-gold)', display: 'flex', alignItems: 'center', gap: '5px', fontSize: '0.9rem' }}>
              ATLETA DEL MES <span style={{ color: '#fff', fontWeight: 'normal', fontSize: '0.8rem' }}>({muroFama.mes_anio})</span>
            </h4>
            ...
            <div key={msg.id} style={{ display: 'flex', flexDirection: 'column', alignItems: isMine ? 'flex-end' : 'flex-start', marginBottom: currentLikes.length > 0 ? '5px' : '0' }}>
              ...
              <div 
                ...
                style={{
                  ...
                  backgroundColor: isMine ? 'rgba(212, 175, 55, 0.15)' : 'rgba(255, 255, 255, 0.05)',
