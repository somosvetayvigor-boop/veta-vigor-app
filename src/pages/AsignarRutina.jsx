...
export default function AsignarRutina({ session }) {
  ...
  const fetchData = async () => {
    ...
    if (perfilData) {
      ...
      const calCompleto = perfilData.calendario_personalizado || {};
      const calEntrenador = calCompleto['entrenador'] || {};
      
      setCalendario(prev => ({ ...prev, ...calEntrenador }));
    }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      calCompleto['entrenador'] = calendario;
  ...
  const handleDeleteRutina = async (rutinaId) => {
    ...
    try {
      ...
      if (changed) {
        ...
        // Save the updated calendar immediately
        const calCompleto = alumno?.calendario_personalizado || {};
        const updatedCalCompleto = { ...calCompleto, ['entrenador']: newCal };
        await supabase
          .from('perfiles')
          .update({ calendario_personalizado: updatedCalCompleto })
          .eq('id', alumnoId);
      }
    } catch (error) {
    ...
    <div className="container" style={{ paddingBottom: '90px', paddingTop: '20px' }}>
      ...
      {alumno && (
        <div style={{ display: 'flex', alignItems: 'center', gap: '15px', marginBottom: '20px' }}>
          <img 
            src={alumno.avatar_url || 'https://via.placeholder.com/60'} 
            style={{ width: '50px', height: '50px', borderRadius: '50%', objectFit: 'cover' }} 
            alt="alumno"
          />
          <div>
            <h1 className="gold-gradient-text" style={{ fontSize: '1.5rem', margin: 0 }}>{alumno.full_name}</h1>
            <p style={{ color: 'var(--text-muted)', margin: 0, fontSize: '0.9rem' }}>{alumno.email}</p>
          </div>
        </div>
      )}
      ...
      <div style={{ background: 'var(--bg-card)', padding: '20px', borderRadius: '16px', marginBottom: '20px', border: '1px solid rgba(255,255,255,0.05)' }}>
        ...
        <button 
          onClick={() => navigate(`/crear-rutina?alumno_id=${alumnoId}`)}
